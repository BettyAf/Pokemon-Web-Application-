...
public class WithdrawChallengeCommand extends ValidatorCommand {
	...
	public void process() throws CommandException {
		try {
			...
			if(user == null) {
				throw new CommandException("Must be logged in to withdraw from a challenge.");
			}
			...
			} else if (challenge.getChallenger().getId() != player.getId()) {
				throw new CommandException("Cannot withdraw from a challenge that you did not issue.");
			} else if (challenge.getStatus() != ChallengeStatus.Open.ordinal()) {
				throw new CommandException("Cannot withdraw from a challenge that is not open.");
			}
								
			Challenge c = ChallengeFactory.registerDirty(challenge.getId(), version, challenge.getChallenger(), challenge.getChallengee(), challenge.getChallengerDeck(), ChallengeStatus.Withdrawn.ordinal());
